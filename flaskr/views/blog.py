    ...
    def post(self):
        ...
        if error is not None:
            ...
            db.execute(
                ...
                ' VALUES (?, ?, ?, ?, ?, ?)',
                (assetTicker, avaragePaidValue, amount, operation, transactionDate, g.user['id'])
            )
            db.commit()
...
routes.add_url_rule('/<int:id>/delete/', view_func=DeleteTransaction.as_view("DeleteTransaction"))
routes.add_url_rule('/<int:id>/update', view_func=UpdateTransaction.as_view("UpdateTransaction"))
